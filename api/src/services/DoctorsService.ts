...
class DoctorsService {
  ...
  public async getAvailableTimes(doctorId: any, date: any) {
    ...
    // delete busy times
    this.parseApposAndDeleteTimes(
      attendances.map((att: { operation: any }) => att.operation),
      allTimes
    );
  ...
  public async getAvailableDoctors(clinicId: string, start: string) {
    ...
    let busyDoctorIds = await doctorsWithFreeAppos.map((t: any) => t.doctorId);
    ...
    busyDoctorIds = busyDoctorIds.concat(
      doctorsWithConfirmedAppos.map((t: any) => t.doctorId)
    );
    ...
    busyDoctorIds = busyDoctorIds.concat(
      doctorsWithOperations.map((t: any) => t.doctorId)
    );
    ...
    busyDoctorIds = busyDoctorIds.concat(
      doctorsWithLeave.map((t: any) => t.userId)
    );
